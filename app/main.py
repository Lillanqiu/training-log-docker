...
SHARE_SETTINGS_PATH = CONFIG_DIR / "share-settings.json"
# admin 账号默认就是 host。要换 host 改 HOST_USERNAME 环境变量即可。
HOST_USERNAME = os.getenv("HOST_USERNAME", "admin").strip() or "admin"
...
def get_batch_slots() -> asyncio.Semaphore:
    global BATCH_SLOTS
...
async def get_share_settings(request: Request) -> dict[str, Any]:
    """共享设置：host 看到完整内容，其他人只看到布尔开关 + 自己有权读到的内容。"""
    user = require_current_user(request)
    settings = load_share_settings()
    is_host = is_host_user(user)
    response = {
        "is_host": is_host,
        "host_username": settings["host_username"],
        "share_api": bool(settings.get("share_api", True)),
        "share_prompt": bool(settings.get("share_prompt", True)),
        "share_skills": bool(settings.get("share_skills", True)),
    }
    if is_host:
        # host 自己看完整内容才能编辑
        response["shared_prompt"] = settings.get("shared_prompt", "")
        response["shared_skills"] = settings.get("shared_skills", "")
    return response


@app.post("/api/share-settings")
async def save_share_settings_endpoint(payload: dict[str, Any], request: Request) -> dict[str, Any]:
    user = require_current_user(request)
    if not is_host_user(user):
        raise HTTPException(status_code=403, detail="只有 host 账号能修改共享设置。")
    current = load_share_settings()
    current["share_api"] = bool(payload.get("share_api", current["share_api"]))
    current["share_prompt"] = bool(payload.get("share_prompt", current["share_prompt"]))
    current["share_skills"] = bool(payload.get("share_skills", current["share_skills"]))
    if isinstance(payload.get("shared_prompt"), str):
        current["shared_prompt"] = payload["shared_prompt"]
    if isinstance(payload.get("shared_skills"), str):
        current["shared_skills"] = payload["shared_skills"]
    # 整组预设列表：校验下结构后存
    if isinstance(payload.get("shared_prompt_presets"), list):
        current["shared_prompt_presets"] = [
            {"name": str(p.get("name", "")).strip(), "prompt": str(p.get("prompt", ""))}
            for p in payload["shared_prompt_presets"]
            if isinstance(p, dict) and str(p.get("name", "")).strip()
        ]
    if isinstance(payload.get("shared_skills_presets"), list):
        current["shared_skills_presets"] = [
            {"name": str(p.get("name", "")).strip(), "skills": str(p.get("skills", ""))}
            for p in payload["shared_skills_presets"]
            if isinstance(p, dict) and str(p.get("name", "")).strip()
        ]
    save_share_settings(current)
    return {"saved": True}


@app.get("/api/shared-defaults")
async def get_shared_defaults(request: Request) -> dict[str, Any]:
    """非 host 用户进来时拉这个端点，前端按开关情况预填字段。"""
    user = require_current_user(request)
    settings = load_share_settings()
    is_host = is_host_user(user)
    bundle: dict[str, Any] = {
    ...
    if is_host:
        # host 本人不用预填，自己就是源头
        return bundle
    if settings.get("share_prompt"):
        if settings.get("shared_prompt"):
            bundle["custom_prompt"] = settings["shared_prompt"]
            bundle["shared_fields"].append("custom_prompt")
        if settings.get("shared_prompt_presets"):
            bundle["shared_prompt_presets"] = settings["shared_prompt_presets"]
            bundle["shared_fields"].append("shared_prompt_presets")
    if settings.get("share_skills"):
        if settings.get("shared_skills"):
            bundle["custom_skills"] = settings["shared_skills"]
            bundle["shared_fields"].append("custom_skills")
        if settings.get("shared_skills_presets"):
            bundle["shared_skills_presets"] = settings["shared_skills_presets"]
            bundle["shared_fields"].append("shared_skills_presets")
    if settings.get("share_api"):
        host_api = host_api_config()
        # host 的 api key 也分享出去，让别人不用挨个填
        if host_api:
            bundle["api_config"] = {
                "format": host_api.get("format", ""),
                "base_url": host_api.get("base_url", ""),
                "api_key": host_api.get("api_key", ""),
                "model": host_api.get("model", ""),
                "gpu_model": host_api.get("gpu_model", ""),
            }
            bundle["shared_fields"].append("api_config")
    return bundle


@app.delete("/api/recovered/{recovered_id}")
async def delete_recovered_output(recovered_id: str, request: Request) -> dict[str, Any]:
    """从磁盘上删除一个找回项（outputs/ 下的文件夹或文件）。

    找回项的 id 形如 discovered-<16位 sha1 前缀>，由 discovered_output_entry() 用
    sha1(str(path)) 算出。我们直接遍历 OUTPUT_DIR，找出 sha1 匹配的那一项再删。
    """
    require_current_user(request)
    if not recovered_id.startswith("discovered-"):
        raise HTTPException(status_code=400, detail="只能用这个接口删除找回项。")
    target_hash = recovered_id[len("discovered-"):]
    if not target_hash:
        raise HTTPException(status_code=400, detail="找回项 id 不合法。")
    if not OUTPUT_DIR.exists():
        raise HTTPException(status_code=404, detail="outputs 目录不存在。")
    target_path: Path | None = None
    for child in OUTPUT_DIR.iterdir():
        h = hashlib.sha1(str(child).encode("utf-8")).hexdigest()[:16]
        if h == target_hash:
            target_path = child
            break
    if target_path is None:
        raise HTTPException(status_code=404, detail="找不到对应的文件/文件夹，可能已被删除。")
    # 强校验：必须落在 OUTPUT_DIR 下，避免被构造 id 删到别的目录
    try:
        ...
        raise HTTPException(status_code=500, detail="无法解析路径。")
    try:
    ...
    return {"deleted": True, "id": recovered_id, "path": str(target_path)}


@app.post("/api/history/delete-batch")
async def delete_generation_history_batch(payload: dict[str, Any], request: Request) -> dict[str, Any]:
    """批量删除历史记录。
    payload: {"ids": ["...", "discovered-...", ...]}

    - 普通历史 id（不以 discovered- 开头）：从 history JSON 移除
    - 找回项 id（discovered-xxxx）：从 outputs/ 真删对应的文件夹/文件
    返回每一项的处理结果。
    """
    user = require_current_user(request)
    ...
    # 1) 把 discovered-* 和普通 id 分开
    discovered_ids = [eid for eid in ids if eid.startswith("discovered-")]
    regular_ids = [eid for eid in ids if not eid.startswith("discovered-")]

    print(f"[delete-batch] user={user.get('username')} total={len(ids)} discovered={len(discovered_ids)} regular={len(regular_ids)}", flush=True)
    print(f"[delete-batch] first few ids: {ids[:5]}", flush=True)

    results: list[dict[str, Any]] = []

    # 2) 找回项：扫一次 OUTPUT_DIR 建 hash -> path 索引，避免每个 id 都扫一次
    if discovered_ids and OUTPUT_DIR.exists():
        index: dict[str, Path] = {}
        for child in OUTPUT_DIR.iterdir():
            h = hashlib.sha1(str(child).encode("utf-8")).hexdigest()[:16]
            index[h] = child
        print(f"[delete-batch] OUTPUT_DIR has {len(index)} entries, sample: {list(index.items())[:3]}", flush=True)
        print(f"[delete-batch] discovered hashes we want: {[eid[len('discovered-'):] for eid in discovered_ids[:5]]}", flush=True)
        for eid in discovered_ids:
            target_hash = eid[len("discovered-"):]
            target = index.get(target_hash)
            if target is None:
                results.append({"id": eid, "ok": False, "reason": "not_found"})
                continue
            try:
                resolved = target.resolve()
                root = OUTPUT_DIR.resolve()
                if root not in resolved.parents and resolved != root:
                    results.append({"id": eid, "ok": False, "reason": "outside_outputs"})
                    continue
                if target.is_dir():
                    shutil.rmtree(target)
                else:
                    target.unlink(missing_ok=True)
                results.append({"id": eid, "ok": True})
            except OSError as exc:
                results.append({"id": eid, "ok": False, "reason": str(exc)})
    elif discovered_ids:
        for eid in discovered_ids:
            results.append({"id": eid, "ok": False, "reason": "outputs_missing"})

    # 3) 普通历史 id：一次性 load + 过滤 + save，避免多次写文件
    if regular_ids:
        regular_set = set(regular_ids)
        with _HISTORY_FILE_LOCK:
            ...
            for item in items:
                ...
                if item_id in regular_set and can_view_history_entry(item, user):
                    removed_ids.add(item_id)
                    continue
                kept.append(item)
            if removed_ids:
                save_generation_history(kept)
        for eid in regular_ids:
            results.append({"id": eid, "ok": eid in removed_ids, "reason": "" if eid in removed_ids else "not_found_or_forbidden"})

    total_ok = sum(1 for r in results if r["ok"])
    fails = [r for r in results if not r["ok"]]
    print(f"[delete-batch] done. ok={total_ok}/{len(ids)} failed={len(fails)} sample_fail={fails[:3]}", flush=True)
    return {"deleted": total_ok, "total": len(ids), "results": results}


@app.delete("/api/history/{entry_id}")
async def delete_generation_history(entry_id: str, request: Request) -> dict[str, Any]:
    """删除单条历史记录（不删除磁盘上的文档文件本身）。"""
    user = require_current_user(request)
    if str(entry_id).startswith("discovered-"):
        # discovered-* 是扫描 outputs/ 目录生成的“找回项”，不在历史 JSON 里，无法删除。
        raise HTTPException(status_code=400, detail="找回项不能从历史里删除，请直接清理输出目录。")
    removed = delete_generation_history_entry(entry_id, user)
    if not removed:
        raise HTTPException(status_code=404, detail="历史记录不存在或没有权限删除。")
...
async def discard_resume_job(job_id: str, request: Request) -> dict[str, Any]:
    ...
    if not state or not can_view_resume_job(state, user):
        raise HTTPException(status_code=404, detail="任务不存在。")
    # 正在跑的任务先取消，再删，避免删掉之后任务还在写文件造成混乱。
    task = JOB_TASKS.get(job_id)
    if job_id in JOBS and JOBS[job_id].get("status") == "running" and task and not task.done():
        JOBS[job_id]["cancel_requested"] = True
        task.cancel()
    # 真正从磁盘上删掉这个 resume-job 的 state JSON，列表里就看不到了。
    # 已生成的文件夹（output_folder）保留，避免误删用户已经成功的产物。
    path = resume_job_path(job_id)
    try:
        path.unlink(missing_ok=True)
    except OSError:
        # 退化方案：删不了文件就标成 discarded，前端依然会过滤掉。
...
async def download_folder_file(folder: str, filename: str, request: Request) -> FileResponse:
    require_current_user(request)
    if not re.fullmatch(r"[\w\u4e00-\u9fff. \-]+", folder):
        raise HTTPException(status_code=404, detail="File not found.")
...
def load_share_settings() -> dict[str, Any]:
    """读取共享设置。默认 host 只共享自己的 prompt/skills/api，开关全部 true。"""
    defaults = {
        ...
        "shared_skills": "",
        # 整组预设列表（自定义、非内置的）
        "shared_prompt_presets": [],  # [{"name": "...", "prompt": "..."}]
        "shared_skills_presets": [],  # [{"name": "...", "skills": "..."}]
    }
    if not SHARE_SETTINGS_PATH.exists():
        return defaults
    try:
        data = json.loads(SHARE_SETTINGS_PATH.read_text(encoding="utf-8"))
    ...
    merged["host_username"] = defaults["host_username"]
    # 校验列表
    if not isinstance(merged.get("shared_prompt_presets"), list):
        merged["shared_prompt_presets"] = []
    if not isinstance(merged.get("shared_skills_presets"), list):
        merged["shared_skills_presets"] = []
    return merged


def save_share_settings(settings: dict[str, Any]) -> dict[str, Any]:
    CONFIG_DIR.mkdir(parents=True, exist_ok=True)
    SHARE_SETTINGS_PATH.write_text(
        json.dumps(settings, ensure_ascii=False, indent=2), encoding="utf-8"
    )
    return settings


def is_host_user(user: dict[str, str] | None) -> bool:
    if not user:
        return False
    username = str(user.get("username") or "")
    if username == HOST_USERNAME:
        return True
    # 免登录模式下，本机的 guest 即视为 host（一台机器只有一个 guest，自然是这台机的"主人"）。
    # 这样不用建 admin 账号就能控制共享设置。
    if username == "guest" and not is_auth_required():
        return True
    return False


def host_api_config() -> dict[str, str]:
    # 优先用 HOST_USERNAME 的配置；如果不存在（比如免登录模式下没建 admin 账号），
    # 退一步用 guest 的配置——免登录模式下 guest 就是 host。
    candidates = [HOST_USERNAME]
    if not is_auth_required() and "guest" not in candidates:
        candidates.append("guest")
    for username in candidates:
        path = api_config_path_for_user(username)
        if not path.exists():
            continue
        try:
...
def append_generation_history(entry: dict[str, Any]) -> dict[str, Any]:
    # 多用户并发时，先 load 再 save 会丢条目；用一个进程内锁串行掉。
    with _HISTORY_FILE_LOCK:
...
def delete_generation_history_entry(entry_id: str, user: dict[str, str]) -> bool:
    """删除单条历史记录。返回是否真的删除了一条。"""
    if not entry_id:
        return False
    with _HISTORY_FILE_LOCK:
        items = load_generation_history()
        ...
        removed = False
        for item in items:
            if str(item.get("id") or "") == entry_id and can_view_history_entry(item, user):
                removed = True
                continue
            kept.append(item)
        if removed:
...
def list_resume_jobs_for_user(user: dict[str, str]) -> list[dict[str, Any]]:
    ...
    for path in RESUME_JOBS_DIR.glob("*.json"):
        ...
        if total <= 0 or completed >= total or state.get("status") in ("complete", "discarded"):
            continue
...
def create_resume_job_state(
    ...
    # 文件夹名走"可读风格"：6.1-6.9 张三；碰撞时 unique_output_folder 加 -2/-3。
    folder_base = friendly_folder_name(selected_records, str(defaults.get("default_writer") or "")) if selected_records else "批量训练日志"
    output_folder = unique_output_folder(folder_base)
...
def discovered_output_entry(path: Path, downloads: list[dict[str, str]], count: int) -> dict[str, Any]:
    ...
    return {
        ...
        "folder_zip_url": "",
        "record_titles": [],
        "stats": {"duration_seconds": 0, "total_tokens": 0, "input_tokens": 0, "output_tokens": 0},
        "recovered": True,
    }
...
async def run_fill_job(
    ...
    used_ai = can_use_ai(api_config)
    # 续写跳篇修复：之前直接 start = len(generated_records)，但 state.completed 有可能因为
    # 多次 save_resume_job / 任务被强制 kill 等情况，比 generated_records 实际个数更大（少了一条）。
    # 这里取两者的最小值，确保不会跳过任何 selected_records[i]。即使代价是重做最后一篇，
    # 也比漏掉一篇好。
    completed_recorded = int((resume_state or {}).get("completed") or 0)
    actual_done = len(generated_records)
    if completed_recorded > actual_done:
        # 说明 state 标记的进度比实际写的多——有一条被记成完成但 record 没存下。
        # 把 completed 拉回到实际值，避免跳过下一条。
        completed_recorded = actual_done
    start_index = min(min(actual_done, completed_recorded), len(selected_records))
    job["completed"] = start_index
    # 多用户并发：最多同时跑 MAX_CONCURRENT_BATCH_JOBS 个批量任务，超出的排队等待。
    slots = get_batch_slots()
    slot_acquired = False
    if used_ai:
        queued_msg = job.get("message")
        if slots.locked():
            job["message"] = "已排队，等待其他用户的生成完成..."
        await slots.acquire()
        slot_acquired = True
    ...
    try:
        ...
        while pending:
            ...
            chunk = pending[:parallel]
            pending = pending[parallel:]
            first_title = str(chunk[0][1].get("title") or f"training-log-{chunk[0][0]}")
            job["current"] = first_title
            if parallel > 1 and len(chunk) > 1:
                job["message"] = f"并行生成第 {chunk[0][0]}–{chunk[-1][0]}/{len(selected_records)} 篇..."
            else:
                job["message"] = f"正在生成第 {chunk[0][0]}/{len(selected_records)} 篇：{first_title}"
            if resume_state:
...
def attach_partial_batch_result(
    ...
    if not generated_paths or not generated_records:
        return
    package_path = write_generated_package(generated_paths, generated_records, output_package_mode, default_writer)
    job["partial_result"] = {
        "filename": original_filename,
        "used_ai": used_ai,
        "batch": True,
...
def write_generated_package(
    ...
    # 用友好命名（6.1-6.9 张三）；没有 writer 就只剩日期段。
    root_folder = friendly_folder_name(records, writer) if records else safe_filename(package_title_for_records(records))
...
def unique_output_folder(root_folder: str) -> Path:
    """输出文件夹命名：清爽风格（"6.1-6.9 张三"），不再带 uuid 前缀。
    碰到同名就追加 -2 / -3 后缀，永不覆盖已经生成好的内容。"""
...
async def call_ollama_raw_with_stats(base_url: str, model: str, prompt: str, api_key: str = "") -> dict[str, Any]:
    ...
    load_s = round(float(payload.get("load_duration") or 0) / 1_000_000_000, 2)
    prompt_eval_s = round(float(payload.get("prompt_eval_duration") or 0) / 1_000_000_000, 2)
    eval_s = round(float(payload.get("eval_duration") or 0) / 1_000_000_000, 2)
    print(
        f"[ollama timing] model={model} "
        f"load={load_s}s prompt_eval={prompt_eval_s}s eval={eval_s}s total={round(duration_seconds,2)}s "
        f"prompt_tokens={payload.get('prompt_eval_count')} gen_tokens={payload.get('eval_count')}",
        flush=True,
    )
...
def friendly_folder_name(records: list[dict[str, Any]], writer: str) -> str:
    """生成可读的文件夹名，例如 "6.1-6.9 张三" / "6.1 张三" / "6.1等26天 张三"。

    - 月日用一位数表达（"6.1" 而不是 "06.01"），更符合中文习惯
    - 填写人不为空时，跟在日期后面用一个空格隔开
    - 返回值已经经过 safe_filename，可直接当目录名
    """
    dates_raw = [parse_record_date(record_date_value(r)) for r in records]
    dates = [d for d in dates_raw if d is not None]
    if dates:
        first = dates[0]
        last = dates[-1]
        if len(dates) == 1:
            date_part = f"{first.month}.{first.day}训练日志"
        elif are_continuous_dates(dates):
            date_part = f"{first.month}.{first.day}-{last.month}.{last.day}训练日志"
        else:
            date_part = f"{first.month}.{first.day}等{len(dates)}天训练日志"
    else:
        date_part = f"{output_title_for_records(records)}训练日志"
    writer = (writer or "").strip()
    name = f"{date_part} {writer}" if writer else date_part
